...
router.post("/register", async ctx => {
  let { username, password, phone, email, sms } = ctx.request.body;
...
router.post("/findPwd", async ctx => {
  ...
  if (user) {
    let transporter = await nodemailer.createTransport({
      ...
      auth: {
        user: QQMailbox,
        pass: QQAuthorizationCode
      }
    });

    let mailOptions = {
      from: QQMailbox, // 从哪个邮箱发送
      to: email,
      subject: "找回密码", // 标题
      text: `这里是【晓晓在线】，您用户名为${user.username}的密码是${user.password}`
      // cc: '抄送',
      // bcc: '私密发送'
    };
